...
import ChooseDialog from '../cmps/ChooseDialog';
...
export default function PersonDetails() {
  ...
  const [isChooseLoaderOpen, setIsChooseLoaderOpen] = useState(false);
  ...
  return (
    <section className="person-details main-container">
      ...
      <div className={`personal-details-main flex`}>
        ...
        <Tabs
          className={'react-tabs flex-1 flex column space-between'}
          defaultIndex={utilService.getCurrDay()}
        >
          <TabList className={'react-tabs__tab-list flex'}>
            {WEEK_DAYS.map((day, idx) => (
              <Tab className={'react-tabs__tab flex-1'} key={idx}>
                {day}
              </Tab>
            ))}
          </TabList>
          ...
          <div className="actions-container flex column gap-16">
            <button
              className="contrast-color"
              onClick={() => setIsChooseLoaderOpen(true)}
            >
              בא לי משהו אחר!
            </button>
            <button onClick={() => setIsWeekEditDialogOpen(true)}>
              ערוך את ימי השבוע
            </button>
          </div>
        </Tabs>
      ...
      {/* Edit week dialog */}
      <AppDialog
        isDialogOpen={isWeekEditDialogOpen}
        onCloseDialog={() => setIsWeekEditDialogOpen(false)}
      >
        <PersonWeekEdit
          person={person}
      ...
      {/* Choose loader Dialog */}
      {isChooseLoaderOpen &&
        <AppDialog
          isDialogOpen={isChooseLoaderOpen}
          onCloseDialog={() => setIsChooseLoaderOpen(false)}
        >
          <ChooseDialog
            person={person}
          />
